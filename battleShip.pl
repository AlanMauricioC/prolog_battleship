...
crearBarcos(N,L,T):- N1 is N-1,
							...
							asserta(barco(N,Tam,Dir,Fini,Cini,0,'f')),						
...
ataque(T0,T1,F,C):- 
	...
	ataque_aux(0,0,F,C,T0,T1,N).

ataque_aux(_,_,_,_,T,T,0):-!.
ataque_aux(F,C1,F,C,T0,T1,N):-
	C1<C, 
	C2 is C1+1,
	N \=0,
	ataque_aux(0,C2,F,C,T0,T1,N).

ataque_aux(F1,C1,F,C,T0,T1,N):-
	F1<F, 
	C1<C,
	F2 is F1+1,
	N \=0,
